        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    @endif
                    <form class="m-t-30" action="{{route('updatePortfolio', $port->id)}}" method="post" enctype="multipart/form-data" id="add_portfolio">
                        @csrf
                        ...
                        <div class="form-group">
                            <label for="portfolio_category_id">Portfolio Category Id</label>
                            <select name="portfolio_category_id" id="" class="form-control">
                                @foreach($categories as $category)
                                    <option value="{{$category->id}}" @if($category->id == $port->portfolio_category_id) selected @endif>{{$category->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="file" class="form-control" id="image" name="image" value="{{$port->image}}">
                        </div>
